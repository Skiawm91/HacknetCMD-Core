...
tuple<string, string, int, vector<string>, vector<int>, bool> hnfcOS::Network::List() { // Name, Type, Ports, PortNames, PortNumber, Shell?
    ...
    if (parent->targetIP == "192.168.0.11") return { "Test Laptop", "LAPTOP", 1, vector<string>{"SSH"}, vector<int>{22}, false };
    if (parent->targetIP == "192.168.0.12") return { "Test Server", "SERVER", 1, vector<string>{"SSH"}, vector<int>{22}, false };
    if (parent->targetIP == "192.168.0.13") return { "Test Phone", "PHONE", 1, vector<string>{"SSH"}, vector<int>{22}, false };
    return {"", "", 0, {}, {}, false};
}